...
const CustomerQueryAdminEmail = ({AdminQuery}) => {
    const [bannerImage, setBannerImage] = useState(null); // State for the banner image
    const [subject, setSubject] = useState('');
    const [body, setBody] = useState('');
  
    useEffect(() => {
        setSubject(AdminQuery?.subject);
        setBody(AdminQuery?.body);
      }, [AdminQuery]);
    ...
    const handleSubmit = async (e) => {
      ...
      formData.append('mailType', "customer-query-admin-email"
      );
...
export default CustomerQueryAdminEmail