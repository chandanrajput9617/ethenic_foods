...
const CustomerQueryEmail = ({UserQuery}) => {
    const [bannerImage, setBannerImage] = useState(null); // State for the banner image
    const [subject, setSubject] = useState('');
    const [body, setBody] = useState('');
  
    useEffect(() => {
        setSubject(UserQuery?.subject);
        setBody(UserQuery?.body);
      }, [UserQuery]);
    ...
    const handleSubmit = async (e) => {
      ...
      formData.append('mailType', "customer-query-email"
      );
...
export default  CustomerQueryEmail